...
# -*- coding: utf-8 -*-
"""
Test autonome pour le capteur de température LM70 via SPI.

Ce script est conçu pour fonctionner indépendamment du reste du projet.
Il configure directement le SPI et le GPIO nécessaire pour communiquer avec le LM70.
...
try:
    ...
    import spidev
    logger.info("Packages requis trouvés: RPi.GPIO et spidev")
except ImportError as e:
    logger.error(f"Package manquant: {e}")
    logger.error("Installez les packages requis avec: pip install RPi.GPIO spidev")
    sys.exit(1)
...
def test_lm70():
    ...
    try:
        ...
        spi = spidev.SpiDev()
        spi.open(0, 0)  # Bus 0, Device 0 (CM5)
