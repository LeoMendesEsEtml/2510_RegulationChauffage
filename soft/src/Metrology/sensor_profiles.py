...
def _ntc_sh_from_two_points(t1_c: float, r1_ohm: float, t2_c: float, r2_ohm: float):
    # Steinhart-Hart reduit: 1/T = A + B*ln(R), C = 0
    t1_k = t1_c + 273.15
    t2_k = t2_c + 273.15
    B = (1.0 / t1_k - 1.0 / t2_k) / (log(r1_ohm) - log(r2_ohm))
    A = 1.0 / t1_k - B * log(r1_ohm)
    # r0_ohm pris a 25 degC pour coherence des NTC
    t0_k = 25.0 + 273.15
    ln_r25 = (1.0 / t0_k - A) / B
    r25 = exp(ln_r25)
    return A, B, 0.0, r25
...
# -------------------------
# Profils PTC
# Points utilises: T1 = -15 degC, T2 = 40 degC
# -------------------------
...
SENSOR_DB = {
    # PTC : r0_ohm calcule a 0 degC ; alpha en [1/degC]
    ...
    # NTC : Steinhart-Hart 2 parametres (C = 0), r0_ohm a 25 degC
    "AF60": SensorProfile(
