# -*- coding: utf-8 -*-
"""
Driver pour le convertisseur analogique-numérique ADS124S08 de Texas Instruments.

Fonctionnalités implémentées:
- Communication SPI pour lecture/écriture des registres
- Configuration des canaux différentiels
- Gestion du gain programmable (PGA)
- Configuration des sources de courant IDAC
- Mesure ratiométrique avec REFP0/REFN0
- Modes de conversion continue/unique
- Gestion du mode chop pour réduction du bruit

Paramètres clés:
- Résolution: 24 bits
- Vitesse d'échantillonnage: 2.5 à 4000 SPS
- Gain programmable: 1 à 128
- Sources de courant IDAC: 10 µA à 2 mA

Référence: https://www.ti.com/product/ADS124S08
"""

# ---- Importation des modules nécessaires ----
from hw.gpio_cm5 import GPIO          # Interface GPIO pour contrôler les broches
from time import sleep, monotonic     # Fonctions de temporisation précises
from typing import List, Optional     # Types pour le typage statique

# ---- Commandes SPI (selon datasheet section 8.5.1) ----
# Format des commandes:
# RESET (0000 0110): Réinitialise tous les registres
# START (0000 1000): Démarre les conversions
# STOP  (0000 1010): Arrête les conversions
# RDATA (0001 0010): Lit la dernière conversion
# RREG  (0010 rrrr): Lit le registre à l'adresse rrrr
# WREG  (0100 rrrr): Écrit dans le registre à l'adresse rrrr
CMD_RESET = 0x06      # Réinitialise l'ADC et tous ses registres
CMD_START = 0x08      # Démarre le cycle de conversion
CMD_STOP  = 0x0A      # Arrête le cycle de conversion
CMD_RDATA = 0x12      # Lecture des données de conversion
CMD_RREG  = 0x20      # Lecture d'un registre (adresse = 5 bits LSB)
CMD_WREG  = 0x40      # Écriture dans un registre (adresse = 5 bits LSB)

# ---- Adresses des registres (section 8.6) ----
REG_ID       = 0x00   # Registre d'identification du composant (lecture seule)
REG_STATUS   = 0x01   # État de l'ADC (power-on-reset, erreurs, etc.)
REG_INPMUX   = 0x02   # Configuration du multiplexeur d'entrée
REG_PGA      = 0x03   # Configuration du gain programmable (1-128)
REG_DATARATE = 0x04   # Vitesse d'échantillonnage (2.5-4000 SPS)
REG_REF      = 0x05   # Sélection de la référence de tension
REG_IDACMAG  = 0x06   # Amplitude du courant IDAC (10µA-2mA)
REG_IDACMUX  = 0x07   # Routage des sources de courant IDAC
REG_VBIAS    = 0x08   # Tension de polarisation des entrées
REG_SYS      = 0x09   # Configuration système (chop, etc.)
REG_GPIOCFG  = 0x0B   # Configuration des GPIO
REG_GPIODIR  = 0x0C   # Direction des GPIO
REG_GPIODAT  = 0x0D   # État des GPIO

# ---- Tables de correspondance ----
# Table des entrées analogiques (section 8.6.3)
AIN_MAP = {
...
# Modes de référence de tension (section 8.6.6)
REF_SEL = {
    # Référence interne 2.5V
    "internal_2V5": 0b000,
    # Mode ratiométrique avec REFP0/REFN0
    "ratiometric_REFP0_REFN0": 0b001,
    # Mode ratiométrique avec REFP1/REFN1
    "ratiometric_REFP1_REFN1": 0b010
}

# Vitesses d'échantillonnage en mode SINC3 (section 8.6.5)
# Note: seules les valeurs communes sont listées
DATA_RATE_CODE = {
    2.5: 0x00,    # Plus lent, meilleure résolution
    5: 0x01,
    10: 0x02,
    ...
    1000: 0x0B,
    2000: 0x0C,
    4000: 0x0D    # Plus rapide, résolution réduite
}
...
class ADS124S08:
    ...
    def __init__(self, spi, cs_pin: int, drdy_pin: int, start_pin: int,
        ...
        Args:
            ...
            cs_pin: Numéro de broche GPIO pour Chip Select
            drdy_pin: Numéro de broche GPIO pour Data Ready
            start_pin: Numéro de broche GPIO pour Start
            a0_pin: Numéro de broche GPIO pour A0 (sélection référence)
            a1_pin: Numéro de broche GPIO pour A1 (sélection référence)
        ...
        self.a0 = a0_pin          # Broche A0 sélection référence
        self.a1 = a1_pin          # Broche A1 sélection référence

        # Configuration des broches GPIO
        # Sorties: CS, START, A0, A1 (état initial haut)
        for pin in [self.cs, self.start, self.a0, self.a1]:
            self.gpio.setup(pin, self.gpio.OUT, initial=1)
            
        # Entrée: DRDY avec pull-up
        self.gpio.setup(self.drdy, self.gpio.IN, 
                       pull_up_down=self.gpio.PUD_UP)

    def set_ref_bank(self, bank: int) -> None:
        """
        Configure le sélecteur de référence TMUX 2 bits.
        
        Les broches A0/A1 sélectionnent la banque de résistances:
          0 -> A1=0, A0=0 : Première banque
          1 -> A1=0, A0=1 : Deuxième banque
          2 -> A1=1, A0=0 : Troisième banque
          3 -> A1=1, A0=1 : Quatrième banque
        ...
        if not 0 <= bank <= 3:
            raise ValueError("L'index de banque doit être entre 0 et 3")

        # Décodage des niveaux logiques
        if bank == 0:
            a1_level = 0
            a0_level = 0
        elif bank == 1:
    ...
    # ---- Méthodes bas niveau SPI ----
    
    def _cs_low(self) -> None:
        """Active le chip select (état bas)."""
        self.gpio.output(self.cs, 0)

    def _cs_high(self) -> None:
        """Désactive le chip select (état haut)."""
    ...
    def _cmd(self, opcode: int) -> None:
        ...
        self._cs_high()                      # Désactive CS

    def _wreg(self, addr: int, data: List[int]) -> None:
        """
        Écrit des données dans un ou plusieurs registres.
    ...
    def _rreg(self, addr: int, n: int) -> List[int]:
        ...
        Format: RREG (0010 aaaa) + count + données
        aaaa = adresse du premier registre (0-31)
        count = nombre de registres - 1
        
        Args:
            addr: Adresse du premier registre
            n: Nombre de registres à lire
        ...
        out = self.spi.readbytes(n)      # Lecture des données
        self._cs_high()
        return out
    ...
    def reset(self) -> None:
        """
        Réinitialise l'ADC avec la commande RESET.
    ...
    def start_continuous(self) -> None:
        """
        Démarre les conversions continues.
        Utilise la commande START (plutôt que la broche).
        """
        self._cmd(CMD_START)
    ...
    def basic_setup(self, pga_gain: int = 1, 
        ...
        """
        Configure les paramètres de base de l'ADC.
        
        Args:
            pga_gain: Gain du PGA (1,2,4,8,16,32,64,128)a
            data_rate_sps: Vitesse en échantillons/seconde
            ref_mode: Mode de référence
            chop: Active/désactive le mode chop
        ...
        # Configuration de la vitesse d'échantillonnage
    ...
    def route_idac(self, current_uA: int = 100,
        ...
        Args:
            current_uA: Courant en µA (10-2000)
            idac1_route: Canal pour IDAC1 ou None pour désactiver
            idac2_route: Canal pour IDAC2 ou None pour désactiver
    ...
    def _wait_drdy(self, timeout_s: float = 0.5) -> None:
        """
        Attend que DRDY passe à l'état bas.
    ...
    def read_once_blocking(self) -> int:
        ...
        Returns:
            int: Valeur convertie (24 bits signés)
            
        Raises:
            TimeoutError: Si pas de données disponibles
        """
        self._wait_drdy()           # Attend données prêtes
        ...
        # Assemblage en entier 24 bits signé
    ...
    def code_to_ratio(self, code: int, gain: Optional[int] = None) -> float:
        ...
        En mode ratiométrique:
        code = (VIN / VREF) * (2^23)/gain
        ratio = code / (2^23) * gain
        
        Args:
            code: Code ADC 24 bits signé
            gain: Gain utilisé (si None, lu depuis le registre PGA)
